<template>
	<MglLayer v-bind="$attrs" :layer-id="layerId!" type="symbol" :source="source" :before="before" :options="options" />
</template>
...
/**
 * Named shim over the generic `<MglLayer type="symbol">`.
 *
 * Kept so existing templates keep working: the props are the ones this component has always had. New
 * code should prefer `<MglLayer type="symbol" :options="…">`, or `useLayer()` for full control.
 *
 ...
 */
defineOptions({ name: 'MglSymbolLayer', inheritAttrs: false });
