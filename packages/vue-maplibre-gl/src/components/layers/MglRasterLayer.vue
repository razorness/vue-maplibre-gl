<template>
	<MglLayer v-bind="$attrs" :layer-id="layerId!" type="raster" :source="source" :before="before" :options="options" />
</template>
...
/**
 * Named shim over the generic `<MglLayer type="raster">`.
 *
 * Kept so existing templates keep working: the props are the ones this component has always had. New
 * code should prefer `<MglLayer type="raster" :options="…">`, or `useLayer()` for full control.
 *
 ...
 */
defineOptions({ name: 'MglRasterLayer', inheritAttrs: false });

const props = defineProps({
	...LayerLib.SHARED.props,
	/** maplibre `layout` properties for this raster layer. Diffed per property, through `setLayoutProperty`. */
	layout: Object as () => RasterLayerSpecification['layout'],
	/** maplibre `paint` properties for this raster layer. Diffed **per property**, so changing one does not restart the others' transitions. */
	paint: Object as () => RasterLayerSpecification['paint'],
	/** Expression deciding which features of the source this layer draws. Applied through `setFilter`. */
	filter: [Boolean, Array] as unknown as () => RasterLayerSpecification['filter']
});
