<template>
	<MglLayer v-bind="$attrs" :layer-id="layerId!" type="heatmap" :source="source" :before="before" :options="options" />
</template>
...
/**
 * Named shim over the generic `<MglLayer type="heatmap">`.
 *
 * Kept so existing templates keep working: the props are the ones this component has always had. New
 * code should prefer `<MglLayer type="heatmap" :options="…">`, or `useLayer()` for full control.
 *
 ...
 */
defineOptions({ name: 'MglHeatmapLayer', inheritAttrs: false });
